...
class ObjectDetection(App):
    ...
    def setup_object_detection_pipeline(self):
        self.object_detection_pipeline = Pipeline("object_detection_pipeline")

        webcam_node = PipelineNode("webcam_capture")
        webcam_node.set_input({"grey_scale": False})
        
        object_detection_node = PipelineNode("object_detection")
        object_detection_node.set_input(
            {"camera_input": webcam_node, 
             "draw_objects": True}
        )
        self.object_detection_pipeline.add_node(webcam_node)
        self.object_detection_pipeline.add_node(object_detection_node)
        self.object_detection_pipeline.add_output_port("detections", object_detection_node)

        self.object_detection_pipeline.start()

    def setup(self):
        self.setup_ui()
        self.setup_object_detection_pipeline()

    def process(self):
        detections = self.object_detection_pipeline.get_output_data("detections")
