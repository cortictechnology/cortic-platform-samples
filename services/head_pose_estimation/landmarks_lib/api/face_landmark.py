...
class FaceLandmark:
    ...
    def __init__(self):
        self.model_path = os.path.dirname(os.path.realpath(__file__)) + '/../../' + cfg.KEYPOINTS.model_path
        self.min_face = 20
        self.keypoints_num = cfg.KEYPOINTS.p_num

        self.coreml_model_path = os.path.dirname(os.path.realpath(__file__)) + '/../../' + cfg.KEYPOINTS.core_ml_model_path
        self.coreml_model = ct.models.MLModel(self.coreml_model_path)
    ...
    def __call__(self, img, bboxes):
        ...
        for i, bbox in enumerate(bboxes):
            ...
            #     inputs = {'images': image_croped}
            #     outputs = self.coreml_model.predict(inputs)
            #     print(outputs['Identity'].shape)
            #     print(outputs['Identity_1'].shape)
            #     print(outputs['Identity_2'].shape)
    ...
    def batch_call(self, image, bboxes):
        ...
        if not self.tflite:
            ...
            inputs = {'images': images_batched.astype(np.float32)}
            outputs = self.coreml_model.predict(inputs)
            landmark = outputs['Identity_2'].reshape((-1, self.keypoints_num, 2))
            states = outputs['Identity']
        else:
