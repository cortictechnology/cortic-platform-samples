...
import numpy as np
import time
from landmarks_lib.api.blaze_face_model import FaceDetector as BlazeFaceDetector
...
class FaceAna():
    ...
    def run(self,image):
        ...
        ###### run detector
            # boxes = self.face_detector(image.copy())
        detections = self.blaze_face_detector.run_inference(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
        faces = np.array([detection['face_coordinates'] for detection in detections])
        if (faces.shape[0] > 0):
            faces[:, 0] *= image.shape[1]
            faces[:, 2] *= image.shape[1]
            faces[:, 1] *= image.shape[0]
            faces[:, 3] *= image.shape[0]
        boxes = faces
