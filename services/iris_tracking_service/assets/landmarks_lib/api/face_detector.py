...
class FaceDetector:
    ...
    def __call__(
        ...
        if not self.tflite:
            ...
            image_fornet, scale_x, scale_y, dx, dy = self.preprocess(
                image, target_height=input_shape[0], target_width=input_shape[1]
            )
        ...
        bboxes = self.py_nms(
            np.array(bboxes[0]), iou_thres=iou_threshold, score_thres=score_threshold
        )

        ###recorver to raw image
        boxes_scaler = np.array(
            [
        ...
        boxes_bias = np.array(
            [dx / scale_x, dy / scale_y, dx / scale_x, dy / scale_y, 0.0],
            dtype="float32",
        )
    ...
    def preprocess(self, image, target_height, target_width):
        ...
        bimage = np.zeros(
            shape=[target_height, target_width, c], dtype=image.dtype
        ) + np.array(cfg.DATA.pixel_means, dtype=image.dtype)
    ...
    def py_nms(self, bboxes, iou_thres, score_thres):
        ...
        while order.shape[0] > 0:
            ...
            iou = intersection / (
                area + (y2_reain - y1_reain) * (x2_reain - x1_reain) - intersection
            )
