...
from assets.custom.core import (  # isort:skip
    detections_to_rect,
...
def from_landmarks_to_depth(
    frame_rgb, eye_landmarks, image_size, is_right_eye=False, focal_length=None, iris_model=None
):
    ...
    eye_contours, iris_landmarks = detect_iris(
        eye_image.copy(), is_right_eye=is_right_eye, iris_model=iris_model
    )
...
def detect_iris(eye_frame, is_right_eye=False, iris_model=None):
    side_low = 64
    eye_frame_low = cv2.resize(
        eye_frame, (side_low, side_low), interpolation=cv2.INTER_AREA
    )

    # model_path = os.path.dirname(os.path.realpath(__file__)) + "/../models/iris_landmark.tflite"
    if (iris_model is None):
        coreml_iris_model_path = os.path.dirname(os.path.realpath(__file__)) + "/../models/iris_landmark/model_coreml_float32.mlmodel"
        iris_model = ct.models.MLModel(coreml_iris_model_path)
    ...
    # print(eye_contours_low.shape)
    # print(iris_landmarks_low.shape)

    eye_frame_low = np.expand_dims(eye_frame_low, axis=0)
    input = {'input_1': eye_frame_low / 127.5 - 1.0}
    coreml_output = iris_model.predict(input)

    # print(coreml_output['output_eyes_contours_and_brows'].shape)
    # print(coreml_output['output_iris'].shape)
    eye_contours_low = np.reshape(coreml_output['output_eyes_contours_and_brows'], (71, 3))
    iris_landmarks_low = np.reshape(coreml_output['output_iris'], (5, 3))
